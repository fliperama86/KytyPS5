...
| `--srt-cache <true\|false>` | `true` | Caches the flattened shader resource table and its descriptor values per frame on the render thread, keyed on the plan, the user-data registers it loads and the shader base, and invalidated by the write generation of every guest page the evaluation read. Off bypasses the cache and takes no page watches at all. | The per-draw SRT evaluation; see the per-frame SRT cache section in [demons-souls-performance.md](demons-souls-performance.md). |
