...
| `--replay-image <path>` | off | After the last loop, writes the presented frame as raw, tightly packed, in the video-out pixel format the guest chose, to `<path>`, with a `<path>.json` sidecar giving width, height, format, bytes per pixel and stride. Demon's Souls presents `A2B10G10R10_UNORM_PACK32`, not an 8-bit format; `_Build/replay-image-compare.py` decodes every format the presenter can produce. | Diffing a replayed frame against the capture screenshot. |
