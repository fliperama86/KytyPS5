...
struct ShaderInfo {
	...
	// Step 1 of docs/sync-points-design.md: the prologue's roots and flattened reads resolve in
	// the upper half of the BdaPagetable binding, the prologue page table, and record a miss in
	// the upper half of FaultBuffer. Both bindings exist already, so the module changes and the
